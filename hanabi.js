...
var dim = 40;
var offline_AI_turn_number = 1;
var offline_vs_AI = true;

function createGrid(grid) {
	$('.griditem').remove();
    var xcount=1, ycount=1, tcount=0;
    var nrows, ncols, html;
    nrows=grid['rows'];
    ncols=grid['cols'];
    html=grid['html']; 
	rows=nrows;
...
createGrid(grids[$('#grid_name').val()]);
			...
			//Horizontal Line
				if (tX < cols - lineLength + 2) {
					if (getText(tX, tY) == currentToken && getText(tX+1, tY) == currentToken && getText(tX+2, tY) == currentToken && getText(tX+3, tY) == currentToken) {
						enlighten(tX, tX+1, tX+2, tX+3, tY, tY, tY, tY, currentToken);
						return true;
					}
				}
			//Vertical Line
				if (tY < rows - lineLength + 2) {
					if (getText(tX, tY) == currentToken && getText(tX, tY+1) == currentToken && getText(tX, tY+2) == currentToken && getText(tX, tY+3) == currentToken) {
						enlighten(tX, tX, tX, tX, tY, tY+1, tY+2, tY+3, currentToken);
						return true;
					}
				}
			//Diagonal Down Line
				if (tX < cols - lineLength + 2 && tY < rows - lineLength + 2) {
					if (getText(tX, tY) == currentToken && getText(tX+1, tY+1) == currentToken && getText(tX+2, tY+2) == currentToken && getText(tX+3, tY+3) == currentToken) {
						enlighten(tX, tX+1, tX+2, tX+3, tY, tY+1, tY+2, tY+3, currentToken);
						return true;
					}
				}
			//Diagonal Up Line
...
function enlighten(x1, x2, x3, x4, y1, y2, y3, y4, currentToken) {
    if (currentToken == 'X') {
        $('#g' + x1 + y1).css('color', "#f00");
        $('#g' + x2 + y2).css('color', "#f00");
        $('#g' + x3 + y3).css('color', "#f00");
        $('#g' + x4 + y4).css('color', "#f00");
    } else {
        $('#g' + x1 + y1).css('color', "#0f0");
        $('#g' + x2 + y2).css('color', "#0f0");
        $('#g' + x3 + y3).css('color', "#0f0");
        $('#g' + x4 + y4).css('color', "#0f0");
    }
}
	...
	if (p[0] != -1) {
		...
		var delay = setInterval(function() {
			if (counter == 1) {
                ...
                var pid = '#g' + p[parseInt((i+1)*Math.random())];
                $(pid).text(AI);
                console.log('AI marking ' + pid);
				//Other Events
				if (judge(4, AI)) {
					//Player Win Algorithm
					turn = 'None';
